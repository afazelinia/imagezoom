...
import { Animated, I18nManager, CameraRoll, Image, View, TouchableWithoutFeedback, TouchableOpacity, Text, TouchableHighlight } from "react-native";
...
var ImageViewer = /** @class */ (function (_super) {
    ...
    function ImageViewer() {
        ...
        // 背景透明度渐变动画
        _this.fadeAnim = new Animated.Value(0);
        // 当前基准位置
        _this.standardPositionX = 0;
        // 整体位移，用来切换图片用
        _this.positionXNumber = 0;
        _this.positionX = new Animated.Value(0);
        _this.width = 0;
        _this.height = 0;
        ...
        */
        ...
        _this.goBack = function () {
            ...
            Animated.timing(_this.positionX, {
        ...
        _this.saveToLocal = function () {
            if (!_this.props.onSave) {
                CameraRoll.saveToCameraRoll(_this.props.imageUrls[_this.state.currentShowIndex || 0].url);
    ...
    ImageViewer.prototype.componentWillReceiveProps = function (nextProps) {
        ...
        if (nextProps.index !== this.state.currentShowIndex) {
            this.setState({
                ...
                _this.jumpToCurrentImage();
                // 显示动画
                Animated.timing(_this.fadeAnim, {
                    toValue: 1,
                    duration: 200
                }).start();
            });
    ...
    ImageViewer.prototype.loadImage = function (index) {
        ...
        Image.getSize(image.url, function (width, height) {
            ...
            try {
                var data = Image.resolveAssetSource(image.props.source);
    ...
    ImageViewer.prototype.getContent = function () {
        ...
        var ImageElements = this.props.imageUrls.map(function (image, index) {
          ...
          {children}
            ...
            switch (imageInfo.status) {
              ...
              <View style={_this.styles.loadingContainer}>{_this.props.loadingRender()}</View>
            </Wrapper>);
                case 'success':
                    if (!image.props) {
                        image.props = {};
        ...
        return (<Animated.View style={{ zIndex: 9 }}>
        <Animated.View style={__assign({}, this.styles.container, { opacity: this.fadeAnim })}>
          ...
          <Animated.View style={__assign({}, this.styles.moveBox, { transform: [{ translateX: this.positionX }], width: this.width * this.props.imageUrls.length })}>
            {ImageElements}
          </Animated.View>
          ...
          {this.props.imageUrls[this.state.currentShowIndex || 0] &&
            ...
            this.props.imageUrls[this.state.currentShowIndex || 0].originUrl && (<View style={this.styles.watchOrigin}>
    ...
    ImageViewer.prototype.render = function () {
        ...
        return (<View onLayout={this.handleLayout} style={__assign({ flex: 1, overflow: 'hidden' }, this.props.style)}>
        {childs}
      </View>);
    };
